...
Заголовок страницы (`<title>`) и `og:title` формируются через `formatScanTitle(scan)` — `«{brand} · {productName}»` если оба есть, иначе один из них, иначе «Бренд не определён». `productType` (тип средства, например «шампунь») идёт мелким подзаголовком в карточке вердикта.
...
- 2026-04-27: Заголовок страницы и `og:title` теперь формируются из `brand` + `productName` через `formatScanTitle`, а не из `productType`.
