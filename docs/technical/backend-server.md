...
Все запросы получают заголовок ответа `X-Request-Id` (4 hex-символа). Этот же ID попадает в логи — для дебага удобно сообщать его при репорте бага. Подробнее — `docs/technical/logging.md`.
...
Создание/отзыв публичной ссылки на скан. Защищены `requireTelegramAuth`. Подробнее — `docs/technical/share.md`.

### `GET /share/:token`
Публичная HTML-страница со сканом. Без `requireTelegramAuth`. Открывается в обычном браузере получателя. Подробнее — `docs/technical/share.md`.

> ⚠️ На проде nginx должен проксировать `/share/*` на Node — иначе будет 404 от nginx (см. `share.md`).
...
- 2026-04-27: Подключено production-grade логирование (access-log, request_id, X-Request-Id, маскировка секретов).
- 2026-04-27: Добавлен публичный роутер `/share/:token` (без auth) и эндпоинты `POST/DELETE /api/scans/:id/share`. Подробности — `share.md`.
